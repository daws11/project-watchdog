...
const STATUS_CONFIG: Record<ConnectionStatus, { label: string; className: string; dotClass: string }> = {
  ...
  paused: {
    label: 'Paused',
    className: 'text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700',
    ...
    <div className="h-full overflow-y-auto">
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          <StatCard label="Channels with Errors" value={totalErrors} color={totalErrors > 0 ? 'red' : undefined} />
        </div>
...
function ChannelCard({ channel, onClick }: { channel: Channel; onClick: () => void }) {
  const color = CHANNEL_COLOR[channel.type]
